...
_SCHED_HEADERS = [
    ...
    ("지원자격 및 기타", "졸업년도",       "제한여부"),      # AF=32
    ("지원자격 및 기타", "졸업년도",       "제한여부"),      # AG=33 (AF2:AG2 병합)
    ("지원자격 및 기타", "기타(추가사항)", "기타(추가사항)"),# AH=34
    # 최저학력기준 (AI~AT = 12열)
    ("최저학력기준", "학생부",  "학생부"),           # AI=35
    ("최저학력기준", "수능",    "국어"),              # AJ=36
    ("최저학력기준", "수능",    "국어"),              # AK=37 (AJ3:AK3 병합)
    ("최저학력기준", "수능",    "수학"),
    ("최저학력기준", "수능",    "수학"),
    ("최저학력기준", "수능",    "영어"),
    ("최저학력기준", "수능",    "탐구영역"),
...
def extract_table_data(table, cap_name: str, schema_keys: list[str]) -> dict[str, list[str]]:
    ...
    if cap_name == "전형일정":
        ...
        # ncols=7: 지원자격유형, 세부지원자격, 학력유형, 공통지원자격, 졸업년도(제한여부), ?, 기타(추가사항)
        # 첨부에서 졸업년도 열이 2개(AF, AG 모두 '제한여부') — 원본표 col4, col5
        key_map = [
        ...
        for dr in data_rows:
            ...
            # 제한여부: col4 또는 col5 (비어있지 않은 것)
            r_val = vals["제한여부_a"] or vals["제한여부_b"]
            for k in ["지원자격유형", "세부지원자격", "학력유형", "공통지원자격", "기타(추가사항)"]:
                if k in result:
                    result[k].append(vals[k])
            # 졸업년도/제한여부 — 스키마에 2열 존재(AF, AG 동일 라벨)
            if "제한여부" in result:
                result["제한여부"].append(r_val)

    elif cap_name == "최저학력기준":
        # ncols=12: 학생부, 수능x10(국어×2,수학×2,영어,탐구×2,제2외/한문,한국사,반영영역수), 세부내용
        # h2 기반: col0=학생부, col1=국어, col2=국어(병합), col3=수학, col4=수학(병합),
        #           col5=영어, col6=탐구, col7=탐구(병합), col8=제2외, col9=한국사, col10=반영영역수
        #           col11=세부내용
        key_order = [
            "학생부(최저)",
            "국어_a", "국어_b",
        ...
        # 스키마 키와 원본 컬럼 순서 직접 매핑 (첨부 기준)
        schema_col_map = {
            "학생부":   (0, "학생부(최저)"),
            "국어":     [(1, "국어_a"), (2, "국어_b")],
            "수학":     [(3, "수학_a"), (4, "수학_b")],
            "영어":     (5, "영어"),
            "탐구영역": [(6, "탐구영역_a"), (7, "탐구영역_b")],
            "제2외국어/한문": (8, "제2외국어/한문"),
            "한국사":   (9, "한국사"),
            "반영영역수": (10, "반영영역수"),
            "세부내용": (11, "세부내용"),
        ...
        for dr in data_rows:
            for k in schema_keys:
                cols = sk_col.get(k, [])
                val = " / ".join(filter(None, [cells.get((dr, c), "") or "" for c in cols])) if cols else ""
                result[k].append(val)
...
_SCHED_KEYS = [
    ...
    # 지원자격
    "지원자격유형", "세부지원자격", "학력유형", "공통지원자격", "제한여부", "제한여부", "기타(추가사항)",
    # 최저
    "학생부", "국어", "국어", "수학", "수학", "영어", "탐구영역", "탐구영역",
    "제2외국어/한문", "한국사", "반영영역수", "세부내용",
]
# 전형요소 — 원본표 컬럼 소제목
_ELEM_KEYS = [
    "적용대상 졸업년도",
...
def build_row(u: dict, 대학명: str, hs: str, he: str, sheet: str) -> tuple[list[tuple], list[str]]:
    ...
    if sheet == "전형일정및방법":
        ...
        # 지원자격 및 기타 (7→6 열 — 제한여부 중복 포함)
        sched_keys_3 = ["지원자격유형", "세부지원자격", "학력유형", "공통지원자격", "제한여부", "기타(추가사항)"]
        ...
        # 최저학력기준 (12→10 열)
        sched_keys_4 = ["학생부", "국어", "국어", "수학", "수학", "영어",
                        "탐구영역", "탐구영역", "제2외국어/한문", "한국사", "반영영역수", "세부내용"]
        unique_keys_4 = ["학생부", "국어", "수학", "영어", "탐구영역",
                         "제2외국어/한문", "한국사", "반영영역수", "세부내용"]
        d4 = extract_table_data(detail_tables.get("최저학력기준"), "최저학력기준", unique_keys_4) \
            if "최저학력기준" in detail_tables else {k: [""] for k in unique_keys_4}
        ...
        values = list(ident_vals) + [
            ...
            _first(d3, "제한여부"), _first(d3, "제한여부"),  # AF, AG 동일 값
            _first(d3, "기타(추가사항)"),
            # 최저 (AI~AT = 12열)
            _first(d4, "학생부"),                                  # AI=35
            _first(d4, "국어"), _first(d4, "국어"),                # AJ=36, AK=37
            _first(d4, "수학"), _first(d4, "수학"),                # AL=38, AM=39
            _first(d4, "영어"),                                    # AN=40
            _first(d4, "탐구영역"), _first(d4, "탐구영역"),        # AO=41, AP=42
            _first(d4, "제2외국어/한문"), _first(d4, "한국사"),    # AQ=43, AR=44
            _first(d4, "반영영역수"), _first(d4, "세부내용"),      # AS=45, AT=46
        ]
