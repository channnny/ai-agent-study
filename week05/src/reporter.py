...
def write_report(results: list[PersonResult], output_path: Path) -> None:
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        _write_styled(writer, summary, "종합",
            "🟢=목표달성 🟡=절반이상 🔴=미달 ⬜=없음.  보는 순서: ①커버리지(173곳 중 몇 곳 시도)→②PK매칭률(같은 행 찾음)→③셀일치율(값 정확).  현재 유찬·이지현은 커버리지·셀 일치율은 목표 달성(90%↑)이나, PK는 전형명 분류 차이로 미달(누락·잉여 시트 참조). PK 85% 도달은 전형명 표준 합의 필요. 임지현은 8개 표본만.")
